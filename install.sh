...
if ! [ -d "$ZSH_CUSTOM_PLUGINS/zsh-autosuggestions" ]; then
    env git clone --depth=1 https://hub.fastgit.xyz/zsh-users/zsh-autosuggestions.git "$ZSH_CUSTOM_PLUGINS/zsh-autosuggestions" || {
...
if ! [ -d "$ZSH_CUSTOM_PLUGINS/zsh-completions" ]; then
    env git clone --depth=1 https://hub.fastgit.xyz/zsh-users/zsh-completions.git "$ZSH_CUSTOM_PLUGINS/zsh-completions" || {
...
if ! [ -d "$ZSH_CUSTOM_PLUGINS/zsh-syntax-highlighting" ]; then
    env git clone --depth=1 https://hub.fastgit.xyz/zsh-users/zsh-syntax-highlighting.git "$ZSH_CUSTOM_PLUGINS/zsh-syntax-highlighting" || {
...
printf "install fzf...\n"
if ! [ -d "$FZF" ]; then
    env git clone --depth=1 https://hub.fastgit.xyz/junegunn/fzf.git "$FZF" || {
